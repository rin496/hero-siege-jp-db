...
MASTER_VERSION="permanent-master-v15.1-standalone-segment-survey"
PREP=0x0C54E830
...
def segment_features(pe, body_begin, body_end):
    calls = []
    b = pe.get(body_begin, body_end)
    i = 0
    while i + 5 <= len(b):
        if b[i] == 0xE8:
            r = body_begin + i
            calls.append((r, (r + 5 + s32(b, i + 1)) & 0xffffffff))
            i += 5
        else:
            i += 1
    preps = [at for at, t in calls if t == PREP]
    rows = []
    for idx, a in enumerate(preps):
        z = preps[idx + 1] if idx + 1 < len(preps) else body_end
        scalls = [(at, t) for at, t in calls if a <= at < z]
        cnt = Counter(t for _, t in scalls)
        rows.append({
            "segment_index": idx,
            "begin": a, "begin_hex": f"0x{a:X}",
            "end": z, "end_hex": f"0x{z:X}",
            "size": z - a,
            "call_count": len(scalls),
            "target": cnt[TARGET],
            "setter": cnt[FIELD_SETTER],
            "prep": cnt[PREP],
        })
    return rows

def parity_profiles(rows):
    out = []
    for parity in (0, 1):
        xs = [r for r in rows if r["segment_index"] % 2 == parity]
        sig = Counter((r["target"], r["setter"], r["call_count"]) for r in xs[:160])
        out.append({
            "parity": parity,
            "segments_considered": min(160, len(xs)),
            "top_signatures": [{"signature": list(k), "count": v} for k, v in sig.most_common(12)]
        })
    return out
...
def main():
    ...
    for idx,r in enumerate(uregs,1):
        ...
        rr=resolve_wrapper(pe,r["wrapper_rva"],
                           max_depth=5 if is_sword else 3,
                           max_nodes=160 if is_sword else 80)
        resolved.append({**r,"resolver":rr})
        best=rr.get("best")
        if best and best.get("bounds"):
            bd=best["bounds"]
            segs=segment_features(pe,bd["begin"],bd["end"])
            surveys.append({
                "name":r["name"],"wrapper_rva_hex":r["wrapper_rva_hex"],
                "best":best,"segment_count":len(segs),
                "parity_profiles":parity_profiles(segs)
            })
            for s in segs:
                all_segments.append({"name":r["name"],"body_rva_hex":best["rva_hex"],**s})
        if is_sword or idx==1 or idx%5==0 or idx==total:
            print(f"Survey progress: {idx}/{total} {r['name']} nodes={rr['nodes_examined']} best={(best or {}).get('rva_hex')}",flush=True)
    ...
    unique_resolved=sum(1 for x in resolved if x["resolver"].get("best"))

    summary={
        "version":MASTER_VERSION,
        "architecture":"standalone-no-wrapper",
        "exe":str(exe),"exe_sha256":sha256(exe),
        "defineitem_registration_count":len(regs),
        "unique_registration_count":len(uregs),
        "unique_resolved_count":unique_resolved,
        "sword_resolved_to_expected_body":sword_found,
        "sword_expected_body_hex":f"0x{SWORD_BEGIN:X}",
        "resolved_unique_bodies":[
            {"name":x["name"],"wrapper_rva_hex":x["wrapper_rva_hex"],
             "best_rva_hex":(x["resolver"].get("best") or {}).get("rva_hex"),
             "best_depth":(x["resolver"].get("best") or {}).get("depth"),
             "prep":((x["resolver"].get("best") or {}).get("counts") or {}).get("prep"),
             "setter":((x["resolver"].get("best") or {}).get("counts") or {}).get("setter"),
             "target":((x["resolver"].get("best") or {}).get("counts") or {}).get("target")}
            for x in resolved
        ],
        "design_status":"segment-survey-only; no segment is promoted to an item record unless a later cross-function rule is validated",
        "next_design_goal":"derive and validate generic record-boundary rules from parity/signature patterns, using Sword as the authoritative anchor"
    }
    savej(out/"master_summary.json",summary)
    savej(out/"unique_resolver.json",resolved)
    savej(out/"unique_segment_survey.json",surveys)
    savej(out/"all_unique_segments.json",all_segments)

    with (out/"unique_function_bodies.csv").open("w",newline="",encoding="utf-8-sig") as f:
        cols=["name","wrapper_rva_hex","best_rva_hex","best_depth","prep","setter","target","nodes_examined"]
        w=csv.DictWriter(f,fieldnames=cols);w.writeheader()
        for x in resolved:
            b=x["resolver"].get("best") or {}; c=b.get("counts") or {}
            w.writerow({"name":x["name"],"wrapper_rva_hex":x["wrapper_rva_hex"],
                "best_rva_hex":b.get("rva_hex"),"best_depth":b.get("depth"),
                "prep":c.get("prep"),"setter":c.get("setter"),"target":c.get("target"),
                "nodes_examined":x["resolver"].get("nodes_examined")})

    with (out/"all_unique_segments.csv").open("w",newline="",encoding="utf-8-sig") as f:
        cols=["name","body_rva_hex","segment_index","begin_hex","end_hex","size","call_count","target","setter","prep"]
        w=csv.DictWriter(f,fieldnames=cols);w.writeheader()
    ...
    print("Sword body regression:",sword_found);print("Elapsed: %.1fs"%(time.time()-t0))
